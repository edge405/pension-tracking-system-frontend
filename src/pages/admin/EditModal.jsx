...
const EditModal = ({ selectedPensioner, onClose, onUpdate }) => {
  const [newAmount, setNewAmount] = useState(
    selectedPensioner ? selectedPensioner.pensionAmount : 2000
  );
  
  if (!selectedPensioner) return null;
  
  const handleUpdate = () => {
    onUpdate(selectedPensioner.id, newAmount);
    onClose();
  };
      ...
      <div className="bg-white rounded-xl p-6 w-full max-w-2xl shadow-2xl border border-gray-200">
        ...
        <div className="space-y-4 mb-6">
          <div className="flex items-center gap-3 p-4 bg-gray-50 rounded-lg">
            ...
            <div>
              <h3 className="font-medium text-gray-800">{selectedPensioner.name}</h3>
              <p className="text-sm text-gray-500">ID: {selectedPensioner.id}</p>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="p-3 bg-gray-100 rounded-lg border border-gray-200 font-semibold">
              ₱{selectedPensioner.pensionAmount.toLocaleString()}
            </div>
          </div>
